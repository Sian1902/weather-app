...
fun WindCard(
    ...
    DetailCard(modifier = modifier, minHeight = 200.dp) {
        // Header row — mirrors CardHeader style but uses the Material Air vector icon
        ...
        Box(
            ...
            // Speed text drawn in Compose so it scales properly
            Canvas(modifier = Modifier.size(130.dp)) {
                drawCompass(windDeg)
            }
            // Overlay speed + unit in the centre circle
...
private fun DrawScope.drawCompass(windDeg: Int) {
    ...
    for (i in 0 until tickCount) {
        ...
        drawLine(
            ...
            start       = Offset(cx + outerR * sin(angleRad).toFloat(), cy - outerR * Math.cos(angleRad).toFloat()),
            end         = Offset(cx + innerR * sin(angleRad).toFloat(), cy - innerR * Math.cos(angleRad).toFloat()),
    ...
    // Arrow — points in the direction wind is blowing TO (windDeg + 180)
    ...
    // Centre circle (opaque fill so text overlay is readable)
    drawCircle(
        color  = Color(0xFF1B2638),
        radius = 28.dp.toPx()
    )
