...
@Composable
fun MoonPhaseCard(
    moonPhaseRaw: Double, modifier: Modifier = Modifier
) {
    val phaseLabel = when {
    ...
    DetailCard(modifier = modifier, minHeight = 160.dp) {
        CardHeader(
            iconRes = R.drawable.ic_moon, title = stringResource(R.string.card_moon_phase)
        )

        Spacer(Modifier.height(12.dp))

        Image(
            painter = painterResource(id = R.drawable.ic_static_moon),
