...
interface WeatherRepository {
    ...
    suspend fun getCurrentWeatherByCoordinates(
        lat: Double, lon: Double, units: String = "metric", lang: String = "en"
    ): CurrentWeatherDto

    suspend fun getCurrentWeatherByCityName(
        cityName: String, units: String = "metric", lang: String = "en"
    ): CurrentWeatherDto
...
sealed class WeatherResult {
    data class Live(
        val current: CurrentWeatherDto, val forecast: ForecastResponseDto
    ) : WeatherResult()

    data class Cached(
        val current: CurrentWeatherDto, val forecast: ForecastResponseDto, val cachedAtEpochMs: Long
    ) : WeatherResult()
}